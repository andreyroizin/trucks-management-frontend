# Phase 1 Requirements – Index

Phase 1 requirements are split into two documents. R2–R5 moved to next phase.

## Client Requirements

**File**: `plans/requirments/Phase 1 Client Requirements final.md`

| ID | Title | Status |
|----|-------|--------|
| R6 | Client Daily Capacity Overview & Requirements Tracking | In progress |
| R8 | Customer-Centric Schedule View (Manager View) | Pending |
| R13 | Client Hour Discrepancy Detection & Automated Email Notifications | Pending |
| R14 | Customer Invoice Preparation – Weekly Cost Visibility (Toll, Tunnel, Fuel) | Pending |
| R17 | Compensation Claims for Traffic Fines & Accidents (Driver Invoice Deductions) | Pending |
| R21 | New Driver Onboarding Workflow with Notifications | Pending |

## Business Requirements

**File**: `plans/requirments/Phase 1 Business Requirements final.md`

| ID | Title | Status |
|----|-------|--------|
| R23 | Potential Driver Management (Company Linking) | Pending |
| R24 | Multiple Contract Types (CAO, ZZP, Inleen, Brief Loonschaal) | ✅ Implemented |
| R25 | Comprehensive Contract System (All Employee Types & Contract Variations) | Pending |
| R26 | Business Partner/Supplier Management (NIWO, Kiwa, Document Tracking) | Pending |
| R28 | Internal Regulations & Policy Document Management (Reglement) | Pending |
| R31 | Comprehensive Driver Profile/Dossier ("Map per Driver") | Pending |
| R32 | Admin Starting Balance Override (Initial Vacation, ADV, Compensation Hours) | Pending |
| R33 | HR Letter Generation System (Leave, Vacation, Warning, Termination) | Pending |
| R34 | Payroll & Tax Reporting (UVS, Payroll Statements, Annual Statement) | Done |
| R35 | Annual Statement Workflows (Leaving vs. Active Employees) | Done |
| R36 | Financial Reporting – Debtors & Creditors List with Turnover | Pending |
| R37 | Invoicing System (Customer Invoices & Self-Billing) | Pending |
| R38 | Quote/Quotation System (Offerte) | Pending |

## Navigation

- **Client requirements**: Planning, capacity, schedules, invoicing, onboarding
- **Business requirements**: HR, contracts, payroll, finance, reporting
