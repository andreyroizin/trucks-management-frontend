...
async function fetchArchivedDriverPeriods(
    ...
    const res = await api.get<ApiResponse<ArchivedPeriodPage>>(
        `/drivers/periods/archived`,       // ⬅️  adjust if your endpoint differs
...
/** react-query hook */
export function useArchivedDriverPeriods(pageNumber: number, pageSize: number) {
    return useQuery({
        queryKey: ['archivedPeriods', pageNumber, pageSize],
